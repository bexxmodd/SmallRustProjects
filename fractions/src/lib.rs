...
impl CommonValue for i32 {
    fn greatest_common_factor(x: i32, y: i32) -> i32 {
        let highest = if x.abs_val() > y.abs_val() {
            x.abs_val()
        } else {
            y.abs_val()
        };
...
impl Fraction {
    ...
    pub fn reduce_form(&mut self) {
        ...
        let gcf = i32::greatest_common_factor(self.numerator, self.denominator);
    ...
    pub fn add(&self, other: &Fraction) -> Fraction {
        let lcm = i32::least_common_multiple(self.denominator, other.denominator);

        let frac_multiple = lcm / self.denominator;
        let other_mutplie = lcm / other.denominator;

        let numerator = frac_multiple * self.numerator + other_mutplie * other.numerator;
    ...
    pub fn sub(&self, other: &Fraction) -> Fraction {
        ...
        let numer = fraction_multiple * self.numerator - other_multiple * other.numerator;
    ...
    pub fn div(&self, other: &Fraction) -> Fraction {
        let mut f = self.mul(&Fraction::new(other.denominator, other.numerator));
        f.reduce_form();
        f
    }
